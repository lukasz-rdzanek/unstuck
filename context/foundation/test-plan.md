...
| 4 | CI integration + Stryker mutation testing | The new tests run in CI; mutation testing grades their quality against a threshold | quality gate over R1–R7 | CI wiring (local Supabase in CI / test DB) + Stryker mutation testing | complete | context/changes/testing-ci-stryker/ |
...
- **CI/CD:** `.github/workflows/ci.yml` runs lint + test (unit/hermetic) + build on push/PR and auto-deploys on merge (`auto-deploy`). The **integration suite now runs in CI** (Phase 4) via a dedicated `integration` job that boots local Supabase (`supabase/setup-cli@v2` + `supabase start`) and runs `npm run test:integration` — **PR-triggered + `workflow_dispatch` only** (not per-commit), **off `deploy.needs`** and secret-free, so it can never block prod deploy. **Stryker** mutation testing is a **selective LOCAL gate** (`npm run test:mutation -- --mutate "<path>"`), run on demand after a risk phase, **not in CI** — see §6 for both recipes.
...
- **CI integration job + Stryker selective mutation gate** — SHIPPED (Phase 4, change `testing-ci-stryker`). Recipes:
  - **CI integration job** (`.github/workflows/ci.yml`): a top-level `integration` job, **sibling of `ci`** and deliberately ADDITIVE — it never gates deploy. Trigger: `if: github.event_name == 'pull_request' || github.event_name == 'workflow_dispatch'` (and `workflow_dispatch:` added to `on:`), so it runs on PRs + on demand, NOT on every master push (those auto-deploy via the fast `ci` job). Steps: `actions/checkout@v4` → `actions/setup-node@v4` (node 22, npm cache) → `npm ci` → `supabase/setup-cli@v2` (`version: 2.98.2`, matched to the `supabase` devDep) → `supabase start` (boots the Docker stack, applies `supabase/migrations/*` + `seed.sql`) → `npm run test:integration` → `if: failure()` `supabase status || true` (diagnostics). **No secrets** (the local stack ships demo keys) and **NOT in `deploy.needs`** — a Docker flake here must never wedge prod deploy. Do NOT add CF tokens or `astro sync` (the job doesn't build). The `ci`/`deploy` jobs are left byte-for-byte unchanged. Verification reality: a CI-config change can't be exercised locally — Phase-1 success was YAML-valid + the underlying commands already green locally; the live run is confirmed on the first PR / dispatch.
  - **Stryker selective mutation gate** — a **local, on-demand** quality check (NOT in CI per CLAUDE.md), proving the Phase 1–3 tests would fail if the code broke. Run narrowed after a risk phase: `npx stryker run --mutate "src/lib/<file>.ts"` (or `npm run test:mutation -- --mutate "<path>"`). Config: `stryker.conf.json` (`testRunner: vitest`, `plugins: ["@stryker-mutator/vitest-runner"]`, `mutate: []` so **scope comes only from `--mutate`**, `concurrency: 2`, `thresholds` high 80 / low 60 / break 50, HTML report under `reports/mutation/`) points at **`vitest.stryker.config.ts`** — a single top-level test config (node env, `include src/**/*.test.ts`, **NO `projects`, NO `globalSetup`**) that mirrors only the "unit" project + reuses the `@`→`./src` + `cloudflare:workers` alias. This keeps Stryker from booting Supabase per mutant. **Sandbox footgun:** Stryker copies the whole project tree into `.stryker-tmp/`; a symlink under `.claude/` made `copyfile` throw `ENOTSUP` — fixed with `ignorePatterns` (`.claude/`, `.agents/`, `context/`, `tests/`, `supabase/`, …) so only `src/` + config are copied. `.stryker-tmp/` + `reports/` are gitignored. **Worked example (safe-next.ts):** initial score **90%** (9/10) with one survivor — the `typeof next === "string"` guard mutated to `true`. Not equivalent: `isSafeNext` is a `next is string` predicate, so a non-string that string-coerces to a path-like value (`["/dashboard"]`) must still be rejected. Killed it with that assertion → **100%** (10/10). Triage rule: kill survivors that represent a real regression; consciously ignore equivalent/cosmetic mutants (don't chase 100% with vibe assertions).
